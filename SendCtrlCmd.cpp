...
BOOL CSendCtrlCmd::Lock(LPCWSTR log, DWORD timeOut) const
{
	this->mutex.lock();
	return TRUE;
}

void CSendCtrlCmd::UnLock(LPCWSTR log) const
{
	this->mutex.unlock();
}
...
DWORD CSendCtrlCmd::SendTCP(wstring ip, DWORD port, DWORD timeOut, CMD_STREAM* sendCmd, CMD_STREAM* resCmd) const
{
...
DWORD CSendCtrlCmd::SendAddloadReserve() const
{
...
DWORD CSendCtrlCmd::SendReloadEpg() const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendReloadSetting() const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendClose() const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendRegistGUI(DWORD processID) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendUnRegistGUI(DWORD processID) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendRegistTCP(DWORD port) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendUnRegistTCP(DWORD port) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendEnumReserve(vector<RESERVE_DATA>* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendGetReserve(DWORD reserveID, RESERVE_DATA* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
DWORD CSendCtrlCmd::SendAddReserve(vector<RESERVE_DATA>* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
DWORD CSendCtrlCmd::SendDelReserve(vector<DWORD>* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
DWORD CSendCtrlCmd::SendChgReserve(vector<RESERVE_DATA>* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
//引数：
// val				[IN]予約一覧
DWORD CSendCtrlCmd::SendEnumTunerReserve(vector<TUNER_RESERVE_INFO>* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
//引数：
// val				[IN]削除するID一覧
DWORD CSendCtrlCmd::SendDelRecInfo(vector<DWORD>* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
DWORD CSendCtrlCmd::SendChkSuspend() const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendReboot() const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendEpgCapNow() const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendEnumReserve2(vector<RESERVE_DATA>* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
	CMD_STREAM send;
...
DWORD CSendCtrlCmd::SendGetReserve2(DWORD reserveID, RESERVE_DATA* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
DWORD CSendCtrlCmd::SendAddReserve2(vector<RESERVE_DATA>* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
DWORD CSendCtrlCmd::SendChgReserve2(vector<RESERVE_DATA>* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
DWORD CSendCtrlCmd::SendAddChkReserve2(RESERVE_DATA* val, WORD* resVal) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
// val				[IN]取得ファイル名
// resVal			[OUT]タイムスタンプ
DWORD CSendCtrlCmd::SendGetEpgFileTime2(wstring val, LONGLONG* resVal) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
DWORD CSendCtrlCmd::SendChgProtectRecInfo2(vector<REC_FILE_INFO>* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
...
DWORD CSendCtrlCmd::SendGUINotifyInfo2(NOTIFY_SRV_INFO* val) const
{
	if( Lock() == FALSE ) return CMD_ERR_TIMEOUT;
	DWORD ret = CMD_ERR;
