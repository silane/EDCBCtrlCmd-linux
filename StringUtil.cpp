...
void WtoA(wstring strIn, string& strOut)
{
	...
	string str;
	for(wstring::iterator itr=strIn.begin();itr!=strIn.end();itr++)
	{
		str.append((char *)&*itr,sizeof(wchar_t));
	}
	iconvpp::converter conv("CP932","wchar_t",true);
	conv.convert(str,strOut);
}
...
void WtoUTF8(wstring strIn, string& strOut)
{
	...
	iconvpp::converter conv("UTF-8","wchar_t",true);
	conv.convert(str,strOut);
}

void WtoUTF16LE(wstring strIn, string& strOut)
{
	if( strIn.empty() == true ){
		strOut="";
		return ;
	...
	iconvpp::converter conv("UTF-16LE","wchar_t",true);
	conv.convert(str,strOut);
}
...
void AtoW(string strIn, wstring& strOut)
{
	...
	int i=0;
	strOut.clear();
	iconvpp::converter conv("wchar_t","CP932",true);
	conv.convert(strIn,str);
	for(string::iterator itr=str.begin();itr!=str.end();i++,itr++)
	{
		int index=i%sizeof(wchar_t);
		tmp[index]=*itr;
		if(index==sizeof(wchar_t)-1)
			strOut.append((wchar_t *)tmp,1);
	}
}
...
void UTF8toW(string strIn, wstring& strOut)
{
	...
	int i=0;
	strOut.clear();
	iconvpp::converter conv("wchar_t","UTF-8",true);
	conv.convert(strIn,str);
	for(string::iterator itr=str.begin();itr!=str.end();i++,itr++)
	{
		int index=i%sizeof(wchar_t);
...
void UTF16LEtoW(string strIn, wstring& strOut)
{
	if( strIn.empty() == true ){
		strOut=L"";
		return ;
	...
	int i=0;
	strOut.clear();
	iconvpp::converter conv("wchar_t","UTF-16LE",true);
	conv.convert(strIn,str);
	for(string::iterator itr=str.begin();itr!=str.end();i++,itr++)
	{
		int index=i%sizeof(wchar_t);
